(function() {
    ...
    describe("Operator case list", function() {
      it("should filter by person", function() {
        modelsRecipe.Case.createRecipe(
          ...
          browser.get(
            CONSTANTS.callcentreBaseUrl + "?ordering=-modified&page=1"
          );
      ...
      it("should allow creating a Case for selected person", function() {
        ...
        expect(
          element(by.css("#personal_details h2.Icon--user")).getText()
        ).toBe(randomName);
      });
  ...
  function searchAndExpect(personQ, numResults) {
    ...
    browser.driver
      .wait(function() {
        return element(
          by.css(".select2-no-results, .select2-result")
        ).isPresent();
      })
      .then(function() {
        expect(element.all(by.css(".select2-results-dept-0")).count()).toBe(
          numResults
        );
      });
  }
