(function() {
  ...
  var diversityTab = element(
    by.cssContainingText(".Tabs-tabLink", "Diversity")
  );
    ...
    describe("Operator diversity monitoring", function() {
      it("should not allow access without personal details object", function() {
        modelsRecipe.Case.createEmpty().then(function(_caseRef) {
          ...
          expect(notices.getText()).toContain(
            "You must add the client's details before completing the diversity questionnaire."
          );
        });
      });
      ...
      it("should complete sexual_orientation questions", function() {
        ...
        saveBtn.click();
        utils.scrollTo(summary);
        expect(summary.getText()).toContain(
          "The client has completed diversity monitoring."
        );
        expect(diversityTab.getAttribute("class")).toContain(
          "Icon--solidTick Icon--green"
        );
      });
  ...
  function chooseOption(name, value) {
    var field = element(
      by.css('[type="radio"][name="' + name + '"][value="' + value + '"]')
    );
