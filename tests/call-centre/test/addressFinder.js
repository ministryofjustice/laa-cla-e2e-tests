(function() {
  ...
  var addresses = element.all(
    by.repeater("address in addresses | filter:address_search")
  );
  ...
  describe("addressFinder", function() {
    ...
    describe("Operator", function() {
      xit("should be able to search for an address on enter", function() {
      ...
      xit("should return 1 result for SW1H 9AJ", function() {
      ...
      xit("should not fill postcode and street if cancelled", function() {
        addresses
          .get(0)
          .element(by.css('input[type="radio"]'))
          .click();
        cancelBtn.click();

        expect(element(by.name("postcode")).getAttribute("value")).toBe(
          enteredPostcode
        );
        expect(element(by.name("street")).getAttribute("value")).toBe("");
      });

      xit("should be able to search an address on button click", function() {
        searchBtn.click();
      ...
      xit("should fill postcode and street with selected option", function() {
        addresses
          .get(0)
          .element(by.css('input[type="radio"]'))
          .click();
        ...
        expect(
          element(by.exactBinding("personal_details.postcode")).getText()
        ).toBe(returnedPostcode);
        expect(
          element
            .all(by.binding("personal_details.street"))
            .get(0)
            .getText()
        ).toBe("Ministry of Justice\n102 Petty France\nLondon");
      });
    });
  });
})();
