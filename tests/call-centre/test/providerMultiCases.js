(function() {
  ...
  describe("providerMultiCases", function() {
    describe("An operator", function() {
      ...
      xit("should create cases as operator and assign then to a provider", function() {
    ...
    describe("A provider", function() {
      ...
      xit("shouldnt't be able to split if the validation fails", function() {
      ...
      xit("should be able to split a case and assign it to same provider if they can deal with that category of law", function() {
      ...
      xit("should be able to split a case and assign it back to an operator", function() {
  ...
  function fillInSplitForm(categoryVal, internal) {
    splitForm
      .element(by.css('[name="category"] option[value="' + categoryVal + '"]'))
      .click();
    splitForm
      .element(by.css('[name="matter_type1"] option:last-child'))
      .click();
    splitForm
      .element(by.css('[name="matter_type2"] option:last-child'))
      .click();
    splitForm
      .element(by.css('[name=internal][value="' + internal + '"]'))
      .click();
  ...
  function assertLogText(text) {
    expect(
      element
        .all(by.css(".CaseHistory-logItemNotes:first-child"))
        .get(0)
        .getText()
    ).toBe(text);
  }
  ...
  function assertError(error) {
    expect(
      splitForm
        .element(by.cssContainingText(".Error-message", error))
        .isDisplayed()
    ).toBe(true);
  }
})();
