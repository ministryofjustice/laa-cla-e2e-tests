(function() {
  ...
  var zeroIncomeNotice = element(
    by.cssContainingText(".Notice", "The system is showing me that:")
  );
  ...
  var specificBenefits = element(
    by.name("your_details-specific_benefits-universal_credit")
  );
    ...
    describe("Operator financial assessment", function() {
      it("cases without specific benefits get grouped question", function() {
        modelsRecipe.Case.createWithGroupedBenefits().then(function(caseRef) {
          browser.get(
            CONSTANTS.callcentreBaseUrl + caseRef + "/eligibility/details/"
          );
      ...
      it("should not be allowed when not in scope", function() {
        modelsRecipe.Case.createWithScopeAndEligibility(false).then(function(
          caseRef
        ) {
      ...
      it("should show all income alerts", function() {
        modelsRecipe.Case.createEmptyWithInScopeAndEligible().then(function(
          ...
          var mortgage = element(
            by.name("eligibility_check.you.deductions.mortgage")
          );
          mortgage.clear();
          mortgage.sendKeys(1);
          saveEC();
          ...
          expect(zeroIncomeNotice.getText()).toContain(
            "you and your partner currently have no income"
          );
          expect(zeroIncomeNotice.getText()).toContain(
            "you and your partner currently have negative disposable income"
          );
          expect(zeroIncomeNotice.getText()).toContain(
            "your housing costs exceed one third of your income"
          );
        });
      });
      ...
      it("show only show alerts when sections are complete", function() {
        ...
        var youBenefits = element(
          by.name("eligibility_check.you.income.benefits")
        );
        var partnerRent = element(
          by.name("eligibility_check.partner.deductions.rent")
        );
        var partnerBenefits = element(
          by.name("eligibility_check.partner.income.benefits")
        );
      ...
      it("show no longer show zero and negative income alerts", function() {
        ...
        var earnings = element(
          by.name("eligibility_check.partner.income.earnings")
        );
        earnings.clear();
        ...
        expect(zeroIncomeNotice.getText()).not.toContain(
      ...
      it("show show alerts again when no partner", function() {
        ...
        expect(zeroIncomeNotice.getText()).toContain(
          "you currently have negative disposable income"
        );
        expect(zeroIncomeNotice.getText()).toContain(
          "your housing costs exceed one third of your income"
        );
      ...
      it("show hide alerts when setting income", function() {
        ...
        var earnings = element(
          by.name("eligibility_check.you.income.earnings")
        );
        earnings.clear();
        earnings.sendKeys(3);
        saveEC();
  ...
  function assertTabsShown(num) {
    element
      .all(by.css('ul [ng-repeat="section in sections"]'))
      .then(function(tabs) {
  ...
  function setSpecificBenefits(value) {
    element(
      by.css(
        '[name="your_details-specific_benefits-universal_credit"][value="' +
          value +
          '"]'
      )
    ).click();
  ...
  function fillIncomeFields(section, fieldset, value) {
    ...
    element
      .all(by.css('[data-fieldset="' + fieldset + '"] input[type="number"]'))
      .each(function(el) {
        el.clear();
        el.sendKeys(value);
      });
