(function() {
  ...
  describe("callMeBack", function() {
    describe("An operator", function() {
      ...
      xit("should be able to suspend a case with a CB1 code", function() {
      ...
      xit("should allow stopping a callback", function() {
        // stop callback
        element(
          by.cssContainingText("callback-status a", "Remove callback")
        ).click();
        browser
          .switchTo()
          .alert()
        ...
        expect(
          element(
            by.cssContainingText("callback-status", "Callback scheduled for")
          ).isPresent()
        ).toBe(false);
      ...
      xit("should be able to reschedule a callback with a CB2 code", function() {
        assertCBx(2, cbCaseRef, "Schedule a new callback");
      });

      xit("should be able to reschedule a callback with a CB3 code", function() {
        assertCBx(3, cbCaseRef, "Schedule final callback");
      });

      xit("should only allow cancelling a callback after a CB3", function() {
        ...
        expect(modal.element(by.css("header")).getText()).toBe(
          "Cancel callback"
        );
        expect(
          modal
            .element(by.cssContainingText("div.modal button", "Save"))
            .isPresent()
        ).toBe(false);
        expect(
          modal
            .element(by.cssContainingText("button", "Stop callback"))
            .isPresent()
        ).toBe(true);
        ...
        expect(
          element(
            by.cssContainingText(".ListTable td a", cbCaseRef)
          ).isPresent()
        ).toBe(false);
    ...
    describe("An operator manager", function() {
      ...
      xit("should be able to schedule a CB1 for tomorrow", function() {
        modelsRecipe.Case.createEmpty().then(function(case_ref) {
          cbCaseRef = case_ref;
      ...
      xit("should be able to view future callbacks schedule", function() {
      ...
      xit("should see the callback for tomorrow in the list", function() {
        var cell = element
          .all(by.css(".CallbackMatrix .CallbackMatrix-cell"))
          .get(1);
        var caseLink = caseList.element(
          by.cssContainingText("tbody tr a", cbCaseRef)
        );
        ...
        caseLink.click();
        expect(element(by.exactBinding("::case.reference")).getText()).toBe(
          cbCaseRef
        );
      });
  ...
  function assertCBx(attemptNum, caseref, expectedScheduleTitle) {
    ...
    modal
      .element(by.name("callbackNotes"))
      .sendKeys(expectedCode + " scheduled.");
    ...
    expect(browser.getCurrentUrl()).toContain(CONSTANTS.callcentreBaseUrl);
    expect(
      element(by.cssContainingText(".ListTable td a", caseref)).isPresent()
    ).toBe(false);
