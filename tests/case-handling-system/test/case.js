  ...
  describe("case", function() {
    ...
    describe("A full case", function() {
      ...
      it("should fill in personal details and adaptations", function() {
        enterDetails(
          _.extend({}, CONSTANTS.personal_details.full, CONSTANTS.adaptations, {
      ...
      it("should fill in a third party", function() {
        var thirdParty = _.extend(
          {},
          CONSTANTS.thirdparty,
          CONSTANTS.thirdparty.personal_details
        );
      ...
      it("should have stored all fields after reload", function() {
        ...
        caseRef.getText().then(function(text) {
          ...
          expect(exemptEl.getText()).toContain(
            CONSTANTS.case.remaining.exempt_user_reason
          );
          expect(element(by.name("case.notes")).getAttribute("value")).toBe(
            CONSTANTS.case.required.notes
          );
        });
      });
    });
  ...
  function selectOption(option, field) {
    ...
    field =
      typeof field === "string" || field instanceof String
        ...
        : "adaptations";
    element(
      by.cssContainingText('[name="' + field + '"] option', option)
    ).click();
  }
  ...
  function checkField(model, value) {
    ...
    if (value === true) {
      ...
      expect(
        element(
          by.css(
            '[ng-if="third_party.spoke_to === true || third_party.spoke_to === false"]'
          )
        ).getText()
      ).toContain("has not spoken with client");
    } else if (nonExact.indexOf(model) > -1) {
      expect(
        element
          .all(by.binding(model))
          .get(0)
          .getText()
      ).toContain(value);
    } else {
