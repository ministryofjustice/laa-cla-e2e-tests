...
module.exports = {
  ...
  "Check callback today (next available)": function(client) {
    ...
    if (timeIsMocked) {
      ...
      console.log(
        "MOCKING TIME TO 2015-1-26 9:00 - must be running ./manage.py mockserver"
      );
    }
    if (now.day() !== 0) {
      if (now.hour() < 17) {
        if (
          now.day() === 6 &&
          (now.hour() > 11 || (now.hour() === 11 && now.minute() > 14))
        ) {
          console.log(
            "Today not available after 11.15am on a Saturday, test skipped"
          );
        } else {
          eligibleJourney(client);
          client.getValue('select[name="callback-time-time_today"]', function(
            result
          ) {
  ...
  "Check callback specific day": function(client) {
    ...
    client
      ...
      .getValue("#callback-time-day option:first-child", function(result) {
        ...
        client.getValue(
          "#callback-time-time_in_day option:last-child",
          function(result) {
            var then = moment([
              selectedDate.substr(0, 4),
              parseInt(selectedDate.substr(4, 2)) - 1,
