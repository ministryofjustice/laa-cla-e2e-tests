...
EMPLOYMENT_QUESTIONS.EMPLOYED = EMPLOYMENT_QUESTIONS.EMPLOYED_MANDATORY.concat(
  EMPLOYMENT_QUESTIONS.EMPLOYED_OPTIONAL
);
EMPLOYMENT_QUESTIONS.ALL = EMPLOYMENT_QUESTIONS.EMPLOYED.concat(
  EMPLOYMENT_QUESTIONS.COMMON
);

var other_income_amount =
...
module.exports = {
  ...
  "Context-dependent questions for employment status": function(client) {
    ...
    EMPLOYMENT_QUESTIONS.EMPLOYED.forEach(function(item) {
      client.assert.visible(
        util.format('[name="your_income-%s-per_interval_value"]', item),
        util.format(
          "    - `your_income-%s-per_interval_value` is visible",
          item
        )
      );
    });
  ...
  "Context-dependent text and questions for partner": function(client) {
    EMPLOYMENT_QUESTIONS.EMPLOYED.concat(EMPLOYMENT_QUESTIONS.COMMON).forEach(
      function(item) {
        client.assert.elementNotPresent(
          util.format('[name="partner_income-%s-per_interval_value"]', item),
          util.format(
            "    - `partner_income-%s-per_interval_value` is not present",
            item
          )
        );
      }
    ...
    client
      ...
      .setYesNoFields(
        ["in_dispute", "is_self_employed", "partner_is_self_employed"],
        0
      )
      ...
      .assert.containsText(
        "form fieldset:nth-of-type(1) header",
        "Your money",
        "    - Your money section is present"
      )
      .assert.containsText(
        "form fieldset:nth-of-type(2) header",
    ...
    EMPLOYMENT_QUESTIONS.COMMON.forEach(function(item) {
      client.assert.visible(
        util.format('[name="partner_income-%s-per_interval_value"]', item),
        util.format(
          "    - `partner_income-%s-per_interval_value` is visible",
          item
        )
      );
    });
